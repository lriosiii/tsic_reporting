SELECT     
   ...
   CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedSSN)) AS SSN,
   o.officename, 
   s.graduationyear, 
   M.EmailAddress as MentorEmail, 
   CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate)) AS BirthDate,
   SS.StudentStatusName, 
   GR.GradeLevelName AS CurrentGradeLevel, 
   s.StudentID as STARID, 
   DATENAME(MM, CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate))) AS BirthdayMonth,
   RIGHT(N'0' + CONVERT(nvarchar(2), MONTH(CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate)))), 2) + '_' + DATENAME(MM, CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate))) AS BirthdayNumberMonth,
   S.HomePhoneNumber, 
   S.MobilePhoneNumber, 
   S.WorkPhoneNumber, 
   S.EmailAddress, 
  ...
  ,(Select replace(replace(Left(Main.Mentors,Len(Main.Mentors)-1), char(10), ''), char(13), '')     As "Notes"
   ...
   left join lookups.StudentContactTypes sct on pg.StudentContactTypeID = sct.StudentContactTypeID
    ...
    DATEDIFF(YY, CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate)), GETDATE()) AS StudentAge,
...
LEFT OUTER JOIN Lookups.FamilySituations FS ON S.FamilySituationID = FS.FamilySituationID       
WHERE       S.IsDeleted = 0
