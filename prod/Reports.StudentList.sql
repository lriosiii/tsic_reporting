SELECT     S.FirstName, S.MiddleName, S.LastName, S.LastName + ', ' + S.FirstName AS StudentFullName, CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedSSN)) AS SSN, o.officename, s.graduationyear, M.EmailAddress as MentorEmail, CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate)) AS BirthDate, SS.StudentStatusName,
                      GR.GradeLevelName AS CurrentGradeLevel, s.StudentID as STARID, DATENAME(MM, S.BirthDate) AS BirthdayMonth, RIGHT(N'0' + CONVERT(nvarchar(2), MONTH(S.BirthDate)), 2) 
                      + '_' + DATENAME(MM, CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate))) AS BirthdayNumberMonth, S.HomePhoneNumber, S.MobilePhoneNumber, S.WorkPhoneNumber, S.EmailAddress,
                      CASE WHEN S.Gender = 'M' THEN 'Male' ELSE 'Female' END AS Gender, CNT.CountyName, SAddress.Address1 AS StudentAddress1, 
                      SAddress.Address2 AS StudentAddress2, SAddress.City AS StudentCity, SAddress.StateID AS StudentState, S.PriorityType, CT.ContractTypeName,
					  SAddress.ZipCode AS StudentZipCode, (Select TOP(1) AgencyName From Offices.Contacts oc Where o.OfficeID = oc.OfficeID And oc.IsDeleted = 0 And AgencyName IS NOT NULL) As AgencyName
						  ,(Select replace(replace(Left(Main.Mentors,Len(Main.Mentors)-1), char(10), ''), char(13), '')     As "Notes"
							From(Select distinct ST2.StudentID, 
									(Select convert(varchar(20), SNS.NoteDate, 120) + ' - ' + SNS.Note + '. ' AS [text()]
						 ...
						 (SELECT     TOP (1) sct.StudentContactTypeName
                            ...
                            WHERE      (StudentID = S.StudentID) and (sga.IsDeleted = 0)
							order by sga.SemesterEndDate DESC) AS LastSemesterUnweightedGPA,
							s.CommunityServiceHours,

							SC.SchoolName, 
                      AD.FirstName + ' ' + AD.LastName AS AdvocateFullName, DATEDIFF(YY, CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, s.EncryptedBirthDate)), GETDATE()) AS StudentAge
					  ,M.FirstName + ' ' + M.LastName AS MentorName, m.MobilePhoneNumber as MentorMobile, m.HomePhoneNumber as MentorHomePhone,
                      S.IsHispanic, R.RaceName, SI.InterestName,  0 AS SomeNewField,
                          (SELECT     SUM(DaysUnExcused) AS TotalDaysExcused
                            FROM          Students.Attendance
                            WHERE      (StudentID = S.StudentID)) AS DaysUnExcused, S.OfficeID, S.ContractSignedDate, s.StudentReferenceID, S.HighSchoolDiplomaDate,
							S.ProbationLevelID, PR.ProbationReasonName,
							( 
...
FROM         Students.Students AS S LEFT OUTER JOIN
					  Offices.Offices o on s.OfficeID = o.OfficeID left outer join
					--  offices.Contacts oc on o.OfficeID = oc.OfficeID left outer join
					  ...
					  lookups.ContractTypes ct on s.ContractTypeID = ct.ContractTypeID LEFT OUTER JOIN
					  --Students.Contacts as spc on s.StudentID = spc.StudentID Left Outer Join
                      ...
                      Students.StudentMentors AS SM ON S.StudentID = SM.StudentID AND SM.IsPrimary = 1 AND SM.UnassignedDate IS NULL LEFT OUTER JOIN
					  ...
					  Lookups.Interests SI ON S.InterestID = SI.InterestID LEFT OUTER JOIN
					  Lookups.ProbationReasons PR ON S.ProbationReasonID = PR.ProbationReasonID
