...
WHERE     ((a.Address1 IS NULL AND m.MentorStatusID in (1,11)) OR
                      (m.MentorStatusID in (1,11) AND a.City IS NULL) OR
                      (m.MentorStatusID in (1,11) AND a.StateID IS NULL) OR
                      (m.MentorStatusID in (1,11) AND a.ZipCode IS NULL) OR
                      (m.MentorStatusID in (1,11) AND  CONVERT(varchar, DecryptByKeyAutoCert(cert_ID('Certificate1'), NULL, EncryptedBirthDate)) IS NULL) OR
                      (m.MentorStatusID in (1,11) AND m.HomePhoneNumber IS NULL AND m.MobilePhoneNumber IS NULL AND m.WorkPhoneNumber IS NULL) OR
                      (m.MentorStatusID in (1,11) AND m.EmailAddress IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.firstname IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.LastName IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.Gender IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.SalutationID IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.CountyID IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.RaceID IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.JoinReasonID IS NULL) OR
					 -- (m.MentorStatusID in (1,6)) AND (mAPP.BackgroundCheckPassed IS NULL) OR
					  (m.MentorStatusID in (1,11) AND mapp.BackgroundCheckReceivedDate IS NULL) OR
					  --(m.MentorStatusID in (1,6)) AND (mapp.TrainingCompleted IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.TrainingCompletedDate IS NULL) OR
					  (m.MentorStatusID in (1,11) AND m.FingerPrintDate IS NULL))
