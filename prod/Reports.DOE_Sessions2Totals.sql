SELECT  DISTINCT	
	o.OfficeName, 
	Count(distinct sms.StudentID) as #OfStudentsWithSessions, 
	s.OfficeID
    	--sms.SessionDate,  
	--sms.SessionDuration, 
	--s.StudentID, 
	...
	--c.CountyName, 
	--o.OfficeName, 
	--LEFT(sms.SessionNote, 1) As Note, s.i3ControlGroup, i3StudyGroupMember, MentorStatusName 
...
LEFT JOIN Students.StudentMentors AS sm ON s.StudentID = sm.StudentID --AND (sm.UnassignedDate < '2013-08-01' OR sm.UnassignedDate IS NULL) --AND sm.MentorAssignmentTypeID = 1 
...
WHERE s.StudentStatusID IN (1,3,4,5)
	...
	And SessionDate Between dbo.CurrentQuarterStart() AND dbo.CurrentQuarterEnd()
