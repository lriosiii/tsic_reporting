SELECT     s.LastName + ', ' + s.FirstName AS StudentName, s.MiddleName, m.LastName + ', ' + m.FirstName As MentorName, st.LastName +', '+st.FirstName as AdvocateName, m.MentorID, sm.MentorAssignmentTypeID,
					  s.StudentStatusID, s.StudentReferenceID, s.OfficeID, s.CurrentGradeLevelID, s.ContractSignedDate, ss.StudentStatusName,
...
FROM         Students.Students AS s INNER JOIN
					  ...
					  offices.Staff ST on s.AdvocateID =  st.StaffID Left outer Join
