...
From(Select distinct ST2.StudentID,
           (Select ST1.FirstName + ' ' + ST1.LastName + ', ' AS [text()]
            ...
            Where SMS.StudentID = ST2.StudentID
				--AND (SMS.UnassignedDate > '2013-08-01' OR SMS.UnassignedDate IS NULL)
				--AND SMS.AssignedDate > '2014-06-30'
				AND (SMS.UnassignedDate IS NULL)
				AND (SMS.MentorAssignmentTypeID = 1)
...
SELECT	  S.FirstName as StudentFirstName
		...
		, CASE When (DateDiff(DAY,IsNull(fmsdcte.FirstSessionDate, '1900-02-01'),  IsNull( GetDate(), '1900-01-01')) ) = 0
			   Then 0
		  when fmsdcte.FirstSessionDate >= GetDate() then Convert(Int, Ceiling((IsNull(tmscte.TotalMentorSessions, 0) / 1)))
		  Else
...
--ORDER BY S.CountyID
--And S.OfficeID = 6
--And AssignedDate > '2014-06-30'
--And lmsdcte.LastSessionDate > fmsdcte.FirstSessionDate
--Order By CountyName, StudentLastName, StudentFirstName
