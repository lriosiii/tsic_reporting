WITH -- Common CTE for total active students for the time period (for mentor match rates, etc)
	...
	/*Started with Tranfsers First and then regular students */
		...
		Select case
					...
					WHEN d.istransfer = 1 and d.tdate > dbo.Sep15() AND Convert(date,DATEADD(Day,30,d.tdate)) <= d.firstsession THEN Convert(date,DATEADD(Day,30,d.tdate))
					WHEN d.istransfer = 1 and d.tdate > dbo.Sep15() AND Convert(date,DATEADD(Day,30,d.tdate)) > d.firstsession AND d.firstsession <= dbo.Oct15() THEN dbo.Oct15()
					WHEN d.istransfer = 1 and d.tdate > dbo.Sep15() AND Convert(date,DATEADD(Day,30,d.tdate)) > d.firstsession THEN d.firstsession
	...
	(
		Select MIN(sms.AssignedDate) As AssignedDate,
			Case when Convert (date, GETDATE()) > dbo.Jun1()
		Then dbo.Jun1()
		Else
 ...
 --    From Students.Students ST2) [Main]
	...
	(Select	ss.StudentID
			...
			,tmcte.TotalMonths
			,case when convert(date, getdate()) < dbo.Oct16() then null
